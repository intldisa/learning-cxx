...
public:
    ...
    // TODO: 实现复制构造器
    DynFibonacci(DynFibonacci const & fib_): cache(new size_t[fib_.cached]), cached(fib_.cached) {
        std::copy(fib_.cache, fib_.cache + fib_.cached, cache);
        // for(int i = 0; i <= cached; ++i) cache[i] = fib_.cache[i];
    }
